...
for (let y = 0; y < H; y++) {
  for (let x = 0; x < W; x++) {
    ...
    const chk = (sx + sy) & 1;
    const base = chk ? [90, 90, 100] : [60, 60, 70];
    const a = c[3] / 255;
    const o = (y * W + x) * 4;
    let v0 = c[0] * a + base[0] * (1 - a);
    let v1 = c[1] * a + base[1] * (1 - a);
    let v2 = c[2] * a + base[2] * (1 - a);
    if (sx % 8 === 0) { v0 = v0 * 0.5 + 60; v1 = v1 * 0.5 + 60; v2 = v2 * 0.5 + 80; }
    if (sy % 8 === 0) { v0 = v0 * 0.5 + 60; v1 = v1 * 0.5 + 60; v2 = v2 * 0.5 + 80; }
...
const out = 'screenshots/fullview-' + (file.match(/entity\/(\w+)\.png/)?.[1] || 'tex') + '.png';
