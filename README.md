...
**光照**：天光和方块光分成两个独立通道各自传播，火把亮度 14。地表光按朝向做环境光遮蔽（AO），嫌卡可以按 `O` 一键关掉。树叶是**能透光但每穿一层扣 1 级**，所以树冠底下不是全黑，只是比空地暗几档；贴着地那一层实测平均天光 12.5（最高 15 是叶隙直通天，最低 10 是树冠正下方）。

![林子里的树冠，抬头能看见叶隙后面的天](screenshots/forest.png)
...
| `textest` | 贴图与颜色：sRGB 管线、皮甲染色、生物盒子几何、方块六面各用哪张贴图（42 条） |
| `orechaintest` | 连锁挖矿、砍树、掉落合并、耐久按格数扣 |
| `invtest` | 背包与合成交互、中键选取方块（91 条） |
| `layouttest` | HUD 布局 |
| `bedtest` | 床与睡觉 |
| `soundtest` | 音效调用 |

**没有用任何测试框架**。纯 Node 脚本自己断言、自己打总表，浏览器套件用 CDP 连无头 Chrome（SwiftShader 软件渲染）跑真事件。`tools/` 里其余文件（`lightshot`、`cowmap`、`texmap`、`mobview`、`heroshot`、`forestshot`、`blockshot` 等）是诊断 / 出图工具，不带断言、不参与基线 —— `mobview` 是生物 3D 转台相机，可以逐只、逐块、多机位拍 PNG 核对模型；`heroshot` 出 README 那张封面，固定机位 + 固定一群生物；`forestshot` 进森林找一个「头顶盖着厚树冠」的落脚点，出一张抬头看叶隙的 + 一张林海全景，并报出树冠底下那一层的天光实测值；`blockshot` 铺一块平地摆一排指定方块（透明 / 镂空 / 朝向一眼看全）。三个出图工具都是可复跑的。
...
- **透明分四路材质批次**。方块按透明方式分批建网格：不透明、**镂空**（贴图里带全透明像素 —— 树叶、床，走 `alphaTest: 0.5` 把透明像素直接丢弃）、半透明（玻璃 / 冰 / 水，关 `depthWrite` 再按 `renderOrder` 排）。走 `alphaTest` 而不是透明通道，是因为透明像素被 discard 掉之后深度照样写，后面该挡的东西还挡得住，不用像玻璃那样靠排序。树叶以前被当不透明方块画，整棵树是一块实心绿板；现在叶隙真能看见天。代价是树冠要双面渲染，帧率从 39 掉到 32（按 `O` 关掉 AO 能补回一部分）。
- **方块可以按方向覆盖单面贴图**。`tiles` 只有 `[顶, 侧面, 底]` 三张，四个侧面天然共用同一张。熔炉的炉门只该在一面，所以方块还能给一张 `faces` 表（`px/nx/py/ny/pz/nz`）按键覆盖单面。注意 `tiles[1]` 同时是物品栏图标的来源，改 `faces` 别动 `tiles`。
- **天空的太阳和月亮是「反推 alpha」救回来的**。`sun.png` / `moon_phases.png` 都没有 alpha 通道（逐像素量过，alpha 全是 255），它们是黑底上的发光图，直接当 sprite 贴上去就是一个不透明的黑方块飘在天上。加载时按亮度反推 alpha：`alpha = max(r,g,b)`，亮度低于 48 的一律抹成全透明，再把 rgb 反预乘回去（不除回去的话发光部分会整体变暗，除回去之后黑底上的原画面和之前一模一样，只是黑底变透明了）。

以上几路透明方式摆在一起就是这个样子（玻璃 / 冰 / 水是半透明，床 / 树叶是镂空，熔炉的炉门只在 -z 那一面）：

![透明、镂空、朝向一目了然的方块展示台](screenshots/blocks.png)
...
- 床只有一张侧面贴图，没有原版「枕头低、被子高」的三维造型 —— 它就是一个方块，六个面（俯视那面是镂空的床垫上方空间）。
